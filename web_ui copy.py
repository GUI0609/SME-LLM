...
# Chat completion llm
llm = ChatOpenAI(
    openai_api_key='sk-413qYBqNnPBCQxKYGnmkwZ0JFNfbTerGwEbqvOxCoGysNyrl',
...
class QuerySMEGraph(BaseTool):
    ...
    description = """when the query related citation and write relation, run the cypher in Neo4j Database and get answer
            Node Types:
            Publication: Represents publications with properties like id, title, tt_ab(title+abstract), publicationName, publicationDate, keep(using n.keep='keep' of Where limit the Publication is a SME Publication).
...
chatbot = autogen.AssistantAgent(
    name="chatbot",
    system_message="For QA tasks, only use the functions you have been provided with. Summerize your query result and answer. eply TERMINATE when the task is done.",
